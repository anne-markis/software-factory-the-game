...
describe("parseStartConfig", () => {
  ...
  it("rejects a negative ideas stock", () => {
    expect(() =>
      parseStartConfig({ ...startJson, stocks: { ...startJson.stocks, ideas: -1 } }),
    ).toThrow(/content\/start\.json/);
  });
...
describe("parseDecisions", () => {
  ...
  it("categorizes every shipped decision (required schema field)", () => {
    ...
    expect(defs.find((d) => d.id === "agent-ci-review")!.category).toBe("change-structure");
  });
  ...
  it("parses a modifyRate add targeting discover (shop-card hook)", () => {
    const defs = parseDecisions([
      {
        id: "x", name: "x", description: "x", category: "ship-faster", cost: {}, removable: true,
        effects: [{ type: "modifyRate", target: "discover", op: "add", value: 1.5 }],
      },
    ]);
...
describe("parseChallenges", () => {
  ...
  it("pins the Studio tiny gigs and unique version ladder", () => {
    ...
    const medium = requireContract(defs.find((p) => p.id === "medium-refactor"));
    ...
    const large = requireContract(defs.find((p) => p.id === "large-refactor"));
    ...
    const landing = requireContract(defs.find((p) => p.id === "gig-landing-page"));
    ...
    const v1 = requireContract(defs.find((p) => p.id === "ship-v1"));
...
describe("parseProjects", () => {
  ...
  it("parses optional pursue and rejects a non-boolean", () => {
    ...
    expect(requireContract(startOnly[0]).pursue).toBeUndefined();
    ...
    expect(requireContract(flagged[0]).pursue).toBe(true);
    expect(requireContract(flagged[0]).ideaCost).toBeUndefined();
    expect(() =>
      parseProjects([
        {
          id: "x",
  ...
  it("parses ideaCost on Pursue and rejects it on Start", () => {
    ...
    expect(requireContract(defs[0]).ideaCost).toBe(200);
    expect(() =>
      parseProjects([
        {
          id: "x",
  ...
  it("parses optional stockFlowMods on a project and rejects an unknown key", () => {
    ...
    expect(requireContract(defs[0]).stockFlowMods).toEqual([{ stock: "users", acquirePerDayDelta: 1 }]);
    expect(() =>
      parseProjects([
        {
          id: "x",
  ...
  it("parses an optional requiresReputation and rejects a negative one", () => {
    ...
    expect(requireContract(defs[0]).requiresReputation).toBe(5);
    expect(() =>
      parseProjects([
        {
          id: "x", name: "x", sizePoints: 1, upfrontCost: 0, payoutPerPoint: 1, completionBonus: 0,
