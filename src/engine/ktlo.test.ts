...
import { ktloSeatHold, productFinishRate } from "./ktlo";
...
describe("KTLO", () => {
  it("is absent in Studio and on from Company, including Megacorp", () => {
    ...
    expect(ktloSeatHold(studio.getContent())).toBe(0);
    ...
    expect(ktlo && isPermanentProject(ktlo) && ktlo.seats).toBe(1);
  ...
  it("reserves finish and one seat before contract work, and cards multiply the base", () => {
    ...
    expect(effectiveCapacity(s, e.getContent()) - ktloSeatHold(e.getContent())).toBe(3);

    e.tick();
    expect(s.finishFlow).toBeCloseTo(2.7);
    expect(s.stocks.inProgress).toBeCloseTo(3);
