...
// auto-fit/minmax. Pin a fixed 5-column template so each stat stays in
...
describe("top stats layout", () => {
  it("uses a fixed 5-column grid so each stat owns one track", () => {
    expect(html).toMatch(/\.stats\s*\{[^}]*grid-template-columns:\s*repeat\(5,\s*minmax\(0,\s*1fr\)\)/);
