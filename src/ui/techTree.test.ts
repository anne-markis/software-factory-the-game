...
describe("buildTechTree", () => {
  it("groups the lean Studio content into three chains and five standalone decisions", () => {
    ...
    expect(tree.standalone.map((d) => d.id).sort()).toEqual(
      ["better-tooling", "hack-day", "one-time-product", "subscription", "user-interviews"].sort(),
    );
  });
  ...
  it("assigns the hire chain tiers: basic-dev 0, reviewer 1", () => {
    const tree = buildTechTree(content());
    const chain = tree.chains.find((c) => c.name === "Hire basic developer")!;
    expect(chain.tiers).toHaveLength(2);
    expect(chain.tiers[0].map((d) => d.id)).toEqual(["basic-dev"]);
    expect(chain.tiers[1].map((d) => d.id)).toEqual(["reviewer"]);
  });

  it("assigns the agent chain tiers: agent 0, review-agent harness and orchestration 1", () => {
    const tree = buildTechTree(content());
    const chain = tree.chains.find((c) => c.name === "Add coding agent")!;
    expect(chain.tiers).toHaveLength(2);
    expect(chain.tiers[0].map((d) => d.id)).toEqual(["agent"]);
    expect(chain.tiers[1].map((d) => d.id)).toEqual(["review-agent", "agent-harness", "agent-orchestration"]);
  });
