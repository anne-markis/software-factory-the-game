...
| `autoSchedule` | Recorded on the instance when effects land (after `delayDays`). Each active owner fills the active era's `parallelCopies` (1 when the era omits it). They pursue the first offerable id in `projectIds`, then further copies of that same choice, until they hold that many Plan or in-flight items or the next copy fails a check. They spend `ideaCostFactor` times that project's idea cost, and only when cash covers the upfront cost plus `cashReserve`. An unaffordable id stops the search, so a later cheaper project is not a fallback. `skipWhenBurnExceedsIncome` waits while KTLO plus payroll exceed recurring income (burst sales are ignored). A held slot stays taken from Plan through in-flight. A later owner can fill a copy this one could not afford; the project cap still stops the queue. Removing the hire does not cancel work already queued. Ids must be pursue contracts in the resolved catalog. |
