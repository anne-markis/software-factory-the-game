...
All nine types are one discriminated union (`effectSchema`). Extra keys
...
| `addToStock` | Any stock in the enum; result clamped at 0. Pipeline writes (`backlog` / `inProgress` / `done`) attach to one in-flight `remaining` (engine-picked when several are live; ADR 0009). Extra In Progress above seats spills to Ready next tick. |
